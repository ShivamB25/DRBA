...
# Device will be set based on arguments
device = None
...
def load_model(model_type):
    # Make sure CUDA is initialized with the correct device before loading models
    if torch.cuda.is_available():
        torch.cuda.set_device(device)
        print(f"Setting active CUDA device to: {device}")
...
if __name__ == '__main__':
    ...
    gpu_id = args.gpu  # GPU device ID

    # Set the d
    # evice based on the specified GPU ID
    if torch.cuda.is_available():
